...
def evaluate_prompt(
    ...
    try:
        ...
        max_examples_raw = os.getenv("EVAL_MAX_EXAMPLES", "0").strip()
        max_examples = int(max_examples_raw) if max_examples_raw.isdigit() else 0
        if max_examples > 0:
            examples = examples[:max_examples]

        per_example_sleep = float(os.getenv("EVAL_SLEEP_SECONDS", "1.0"))

        print(f"   Dataset: {len(examples)} exemplos")
        print(f"   Delay entre exemplos: {per_example_sleep:.1f}s")
        if max_examples > 0:
            print(f"   Modo rápido ativo: avaliando até {max_examples} exemplos")
        ...
        for i, example in enumerate(examples, 1):
            ...
            # Evita estouro de limite de requests/tokens por minuto.
            if i < len(examples) and per_example_sleep > 0:
