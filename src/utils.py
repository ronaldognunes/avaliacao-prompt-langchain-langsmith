...
def is_rate_limit_error(error: Exception) -> bool:
    """Detecta erros típicos de limite de taxa/tokens (HTTP 429)."""
...
def invoke_with_retry(
    ...
    *,
    ...
    """
    Invoca um LLM com retry e backoff exponencial com jitter para mitigar 429.
    """
    last_error = None
    ...
    for attempt in range(1, max_attempts + 1):
        try:
            ...
            jitter = random.uniform(0.0, raw_delay * 0.25)
            sleep_time = raw_delay + jitter

            print(
                f"⚠️  Limite detectado (tentativa {attempt}/{max_attempts}). "
                f"Aguardando {sleep_time:.1f}s para retry..."
            )
