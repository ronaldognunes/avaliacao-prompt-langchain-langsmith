...
def _clip_text(text: str, max_chars: int = 2500) -> str:
    """Limita texto para reduzir consumo de tokens no avaliador."""
    text = text or ""
    if len(text) <= max_chars:
        return text
    return text[:max_chars] + "\n...[texto truncado para controle de tokens]"
...
def get_evaluator_llm():
    ...
    if _EVALUATOR_LLM is None:
        _EVALUATOR_LLM = get_eval_llm(temperature=0)
...
def evaluate_f1_score(question: str, answer: str, reference: str) -> Dict[str, Any]:
    ...
    """
    question = _clip_text(question, max_chars=1200)
    answer = _clip_text(answer, max_chars=2500)
    reference = _clip_text(reference, max_chars=2500)
